...
import '../../features/dashboard/data/repositories/schedule_repository.dart';
import '../../features/finance/data/repositories/audit_log_repository.dart';
...
Future<void> setupLocator({required String baseUrl}) async {
  ...
  // --- Schedule & audit log (integrated: Phase 8) ---
  sl.registerLazySingleton<ScheduleRepository>(
    () => useFakes
        ? FakeScheduleRepository(failureSwitch: sl<FakeFailureSwitch>())
        : ApiScheduleRepository(sl<ApiClient>()),
  );

  sl.registerLazySingleton<AuditLogRepository>(
    () => useFakes
        ? FakeAuditLogRepository(failureSwitch: sl<FakeFailureSwitch>())
        : ApiAuditLogRepository(sl<ApiClient>()),
  );
