...
class PlanHistorySection extends StatelessWidget {
        ...
        children: [
          ...
          if (history.isEmpty)
            Padding(
              ...
              child: Text('plan_history_no_entries'.tr(), style: TextStyle(fontSize: 13.sp, color: palette.textSecondary)),
            )
          else ...[
            Row(
              children: [
                Expanded(flex: 2, child: Padding(padding: EdgeInsets.symmetric(horizontal: 16.w), child: Text('plan_history_col_date'.tr(), style: _headerStyle(context)))),
                Expanded(flex: 2, child: Padding(padding: EdgeInsets.symmetric(horizontal: 16.w), child: Text('plan_history_col_old_plan'.tr(), style: _headerStyle(context)))),
                Expanded(flex: 2, child: Padding(padding: EdgeInsets.symmetric(horizontal: 16.w), child: Text('plan_history_col_new_plan'.tr(), style: _headerStyle(context)))),
                Expanded(
                  flex: 2,
                  child: Padding(
                    padding: EdgeInsets.symmetric(horizontal: 16.w),
                    child: Text('plan_history_col_changed_by'.tr(), textAlign: TextAlign.right, style: _headerStyle(context)),
                  ),
                ),
              ],
            ),
            SizedBox(height: 16.h),
            for (final entry in history) ...[_buildRow(context, entry), SizedBox(height: 16.h)],
          ],
        ],
      ),
    );
  ...
  Widget _buildRow(BuildContext context, PlanChangeEntry entry) {

  final palette = context.palette;
    final dateLabel = '${entry.date.year}-${entry.date.month.toString().padLeft(2, '0')}-${entry.date.day.toString().padLeft(2, '0')}';
    return Container(
      decoration: BoxDecoration(color: palette.cardMuted, borderRadius: BorderRadius.circular(16.r)),
      padding: EdgeInsets.all(16.w),
      child: Row(
        children: [
          Expanded(flex: 2, child: Text(dateLabel, style: TextStyle(fontSize: 14.sp, fontWeight: FontWeight.w500, color: palette.textPrimary))),
          Expanded(flex: 2, child: Text(entry.oldPlanLabel, style: TextStyle(fontSize: 14.sp, color: palette.textSecondary))),
          Expanded(
            flex: 2,
            child: Row(
              mainAxisSize: MainAxisSize.min,
          ...
          Expanded(flex: 2, child: Text(entry.changedBy, textAlign: TextAlign.right, style: TextStyle(fontSize: 14.sp, fontWeight: FontWeight.w500, color: palette.textPrimary))),
        ],
      ),
    );
  }
